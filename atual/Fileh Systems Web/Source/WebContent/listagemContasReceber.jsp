...
$(document).ready(function() {
	...
	$('#id_limpar').on('click', function(){
        ...
        $( "#id_fornecedor" ).val('-1');
        $( "#id_tipoConta" ).val('-1');
        chamaServlet('pesquisarContaReceber','ListagemContasReceber.action');
	});
	...
	$('.marcar_nao_recebida').on('click', function(){	
		...
		if ($('#id_contas_selecionadas_hidden').val() == '') {
			...
			document.forms["hidden"].action = "MarcarContasReceber.action";
	...
	$('.marcar_recebida').on('click', function(){
		   $('.hidden_selecionadas').val('');
    ...
    $( "#dialog-form-alerta" ).dialog({
        ...
        buttons: {
          Sim: function() {
        	  ...
        	  chamaServlet('hidden','ApagarContaReceber.action');
        	 $( this ).dialog( "close" ); 
...
<form  id="Filtros" name="pesquisarContaReceber" action="" method="post" onsubmit="">
...
<div>	
	...
	<div>
		<h3 align="left">Contas a Receber </h3>
		...
		<hr></hr>
			...
			<td>
				...
				<input id="id_fl_situacao" name="fl_situacao" type="radio" <%=request.getSession().getAttribute("fl_situacao_recebido").toString()%> value="S" ></input>Recebido
				<input id="id_fl_situacao" name="fl_situacao" type="radio" <%=request.getSession().getAttribute("fl_situacao_naoRecebido").toString()%> value="N" ></input>Não recebido</p>
			</td>
        ...
        <td>Tipo de conta :
			...
			<td>
				<input  type="button" name="commit" onclick="this.form['commit'].disabled=true ;  chamaServlet('pesquisarContaReceber','ListagemContasReceber.action')" value="  Filtrar  "></input>
				<input title="Limpar filtros aplicados..."  type="button" id="id_limpar" value="  Limpar  "> </input>
				<a href="listagemContasReceberImprimir.jsp" target="_blank"> <input  type="button" value="  Imprimir  "> </input></a>
			</td>
		</tr>
		...
		<hr></hr>
			...
			<table title="Para marcar mais de uma conta como paga ou não, clique nas celulas para seleciona-las..." border="1" align="center" class="main_table"
				...
				<tr bgcolor="#006666">
					...
					<th style="padding: 5px">Recebido</th>
				</tr>
				...
				<%  for(int i = 0; i<arrayContasReceber.size();i++){ %>
					...
					<tr onmouseover="setAtributoIdReceitaCell(this)" class="class_linha_tabela">
						...
						<td align="justify">
							<input type="button" value="Editar/Ver" onclick="chamaServlet('hidden','EditarContaReceber.action')" class="button_disable_click_class"/>	
						</td>
						<td align="justify">						
					...
					<c:set var="vl_recebidar" scope="session" value="<%= arrayContasReceber.get(i).getCor_vl_conta_receber() %>" />
					<c:set var="total" value="${total + vl_recebidar}" />  
				<%}; %>
			...
			<div align="center">
				...
				<input type="button" value="Marcar como Não recebida(s)" class="marcar_nao_recebida" />
				<input type="button" value="Marcar como recebida(s)" class="marcar_recebida" />
			</div>
			<hr></hr>
	</div>
