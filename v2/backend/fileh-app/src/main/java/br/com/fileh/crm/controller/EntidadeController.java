...
public class EntidadeController {
    ...
    public ResponseEntity<List<Entidade>> searchEntidades(@RequestParam Long tenantId, @RequestParam String query) {
        ...
        org.springframework.data.jpa.domain.Specification<Entidade> spec = (root, q, cb) -> cb.equal(root.get("usuarioId"), tenantId);
        for (String word : words) {
            if (word.isEmpty()) continue;
            final String pattern = "%" + word.toLowerCase() + "%";
            org.springframework.data.jpa.domain.Specification<Entidade> wordSpec = (root, q, cb) -> cb.like(cb.lower(root.get("nome")), pattern);
