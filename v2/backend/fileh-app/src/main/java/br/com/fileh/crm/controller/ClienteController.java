        ...
        for (String word : words) {
            ...
            org.springframework.data.jpa.domain.Specification<Cliente> wordSpec = (root, q, cb) -> {
                try {
                    ...
                    return cb.or(cb.equal(root.get("id"), id), cb.like(cb.lower(root.get("nome")), pattern), cb.like(cb.lower(root.get("cpf")), pattern));
                } catch (NumberFormatException e) {
                    return cb.or(cb.like(cb.lower(root.get("nome")), pattern), cb.like(cb.lower(root.get("cpf")), pattern));
                }
            };
            spec = spec.and(wordSpec);
        }
        
        List<Cliente> results = new java.util.ArrayList<>(clienteRepository.findAll(spec));
        results.sort((a, b) -> {
